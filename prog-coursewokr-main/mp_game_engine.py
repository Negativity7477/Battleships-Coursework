...
def initialise_player(username:str, algorithm:list = ["custom", "placement.json"]) -> dict:
    ...
    Returns:
        dict:player's dictionary where key is a username and value is a list of board and battleship
    """
    ...
    players[username] = {'board':board, 'ships':battleships}
    #Create the dictionary, key as username, values are board and battleships
    return players
...
def generate_attack(player_board) -> tuple:
    ...
    while old_coordinates:
        ...
        if player_board[random_x][random_y] == "X" or player_board[random_x][random_y] == "O":
            #Checks if we have hit this coordinate before
...
def print_board(board:list):
    ...
    #Loop through both grids and based on the main board we receive, we add it to ascii_rep
    #Meaning we can print an ascii representation of the main board
    for row_idx, row in enumerate(board):
        for col_idx, cell in enumerate(row):
            if cell is None:
                ascii_rep[row_idx][col_idx] = "."
            elif cell == "X":
                ascii_rep[row_idx][col_idx] = "X"
            elif cell == "O":
                ascii_rep[row_idx][col_idx] = "O"
            else:
                ascii_rep[row_idx][col_idx] = "="
...
def ai_opponent_game_loop():
    ...
    global player_board #player_board is used in other functions so must be global
    ...
    while not finished:
        ...
        while old_coords:
            ...
            try:
                if ai_board[attack_coords[0]][attack_coords[1]] in ["X", "O"]:
                    old_coords = True
...
def multiplayer_game_loop():
    ...
    algorithm_and_filename = [algorithm_1, "placement.json"]
    #In order to allow either files to be used, we create a list
    ...
    while not finished:
        ...
        while old_coords:
            ...
            while attack_coords == (-1,-1):
                attack_coords = game_engine.cli_coordinates_input() #Generate attack coordinates
            try:
                if player_board_2[attack_coords[0]][attack_coords[1]] in ["X", "O"]:
                    #Checks to make sure coordinates have not been hit previously
                    old_coords = True
                    print("Please attack new coordinates")
                    #We have to restart the loop if coordinates have been attacked already
                    continue
                else:
                    old_coords = False #Break the loop
            except IndexError:
        ...
        while True:
            ...
            if player_board[attack_coords[0]][attack_coords[1]] in ["X", "O"]:
                print("Please attack new coordinates")
                continue
